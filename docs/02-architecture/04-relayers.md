...
This multi-relayer set is responsible for relaying messages from a source network to a destination network. Relayers are operating with private key share and execution happens on the destination network with [MPC](/docs/02-architecture/02-mpc.md) private key.&#x20;
...
<!-- TODO Relayers can be configured for various verification systems, including [MPC](../02-architecture/02-mpc.md), Spectre's zk verification, and Zipline's optimistic verification. -->
